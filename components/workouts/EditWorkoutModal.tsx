...
interface EditWorkoutModalProps {
  isOpen: boolean
  onClose: () => void
  onSave: (id: string, workout: Partial<Workout>) => void
  workout: Workout | null
}

export default function EditWorkoutModal({ isOpen, onClose, onSave, workout }: EditWorkoutModalProps) {
  ...
  // Popola il form quando l'allenamento cambia
  useEffect(() => {
    if (workout) {
      setFormData({
        title: workout.title,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!validateForm() || !workout) return
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <h2 className="text-xl font-semibold text-gray-900">Modifica Allenamento</h2>
          <button
            onClick={handleClose}
